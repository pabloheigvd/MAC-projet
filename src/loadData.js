        ...
        Promise.all(parsedMovies.map((it) => {
          ...
          documentDAO.getAllMovies().then((movies) => {
            ...
            Promise.all(movies.map((movie) => new Promise((resolve1) => {
              ...
              const addedPromise = [400, 87, 0, 34, 58].flatMap((quantity, index) => shuffle(movies).slice(0, quantity).map((movie) => graphDAO.upsertAdded(users[index].id, movie._id, {
              ...
              Promise.all(addedPromise).then(() => {
                ...
                const likePromise = [280, 34, 98, 254, 0].flatMap((quantity, index) => shuffle(movies).slice(0, quantity).map((movie) => graphDAO.upsertMovieLiked(users[index], movie._id, {
                ...
                Promise.all(likePromise).then(() => {
                  ...
                  const actorsPromise = [300, 674, 0, 45, 36].flatMap((quantity, index) => shuffle(actors).slice(0, quantity).map(([actorId]) => graphDAO.upsertActorLiked(users[index].id, actorId, {
                    rank: Math.floor(Math.random() * 5) + 1,
                    at: new Date(160613000 * 1000 + (Math.floor(Math.random() * 3124) * 1000)),
                  })));
                  Promise.all(actorsPromise).then(() => {
                    ...
                    const genrePromise = [22, 3, 0, 4, 7].flatMap((quantity, index) => shuffle(genres).slice(0, quantity).map(([genreId, actor]) => graphDAO.upsertGenreLiked(users[index].id, genreId, {
                      rank: Math.floor(Math.random() * 5) + 1,
                      at: new Date(160613000 * 1000 + (Math.floor(Math.random() * 3124) * 1000)),
                    })));
                    Promise.all(genrePromise).then(() => {
                      ...
                      const requestedPromise = [560, 12, 456, 25, 387].flatMap((quantity, index) => shuffle(movies).slice(0, quantity).map((movie) => graphDAO.upsertRequested(users[index].id, movie._id, {
                        at: new Date(160613000 * 1000 + (Math.floor(Math.random() * 3124) * 1000)),
                      })));
